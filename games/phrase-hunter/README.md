...
2. ▶ ボタンで曲のワンフレーズを **1秒** だけ聴く(何度でも聴き直し可)
...
これらを踏まえ、「段階的アンロック(Heardle)× ジャンル別曲当てクイズ(イントロ王/うたドン)× コンボスコアアタック」を1画面・タッチのみで遊べる形にした。サイト内の既存ゲーム(破壊アクション・タイミング収集・2人対戦)とジャンルは被らない。

なお当初は「イントロハンター」という名称だったが、iTunesのプレビュー音源は曲の冒頭ではなくサビなどのハイライト部分から始まるため、実態に合わせて「フレーズハンター」に改名した(聴いて当てるゲーム性自体は同じ)。

## 技術構成

- 単一HTML(`index.html`)・依存ライブラリなし。フォントのみGoogle Fonts(Bungee / Zen Kaku Gothic New / Space Mono)
- **楽曲データ**: iTunes Search API(`itunes.apple.com/search`、`media=all&entity=song&attribute=artistTerm&country=JP` でジャンル別アーティストプールから取得)。fetch失敗時はJSONP(8秒タイムアウト付き)にフォールバック(KAITEN `itunes-music-player.html` と同方式)。**`media=music` は使わない**(下記教訓参照)
- **ヒットチャート**: iTunes旧RSS(`itunes.apple.com/jp/rss/topsongs/limit=100/json`、SnapTune `index.html` と同方式)。プレビューURL・ジャケット込みで取得できLookup不要、CORS可・iOS Safariでも通る。失敗時はJ-POPプールへ自動フォールバック
...
- ベストスコアは localStorage(`phrase-hunter-v1`、try/catchフォールバック)
...
- **`media=music` はiOS Safari(WebKit)で全滅する**: `itunes.apple.com/search` に `media=music` を付けると、AppleがWebKit系UAに対して `musics://`(Apple Musicアプリのディープリンク)へリダイレクトし、非HTTPスキームへのクロスオリジンリダイレクトとしてfetchもJSONPも拒否される。Chromeでは起きないためデスクトップのE2Eだけでは検出できなかった。`media=all&entity=song` なら通る(結果は `kind === "song"` でフィルタ)。同一ページ内で動いている既存アプリ(SnapTune)のリクエスト形と差分を取るのが最短の切り分けだった
- Playwright WebKit(iPhoneデバイスエミュレーション)でiOS Safari固有の問題をローカル再現できる。モバイル実機で失敗する報告を受けたら、Chromeだけでなく WebKit でもE2Eを回すこと
- **iTunesプレビューは「イントロ」ではない**: プレビュー音源は曲冒頭ではなくApple側が選んだハイライト(多くはサビ)から始まる。「イントロクイズ」を名乗ると実際の体験と食い違うため、リリース後に名称・文言を「フレーズ」に改めた。素材の特性を確認してからコピーを書くこと
